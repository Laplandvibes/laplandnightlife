...
const overlay: Record<string, CityOverlay> = {
  ...
  yllas: {
    ...
    intro: 'ユッラスには62本のゲレンデがあり、アカスロンポロ（北）とユッラスヤルヴィ（南）の2つの村に分かれています。ゲレンデでつながっていますが、車では20分の距離。夜が賑やかなのはアカスロンポロ側で、アフタースキーはSport Resort Ylläs、その後はTaiga Pub & KitchenとRavintola Otsoへ。メガクラブはありません。レヴィより静かで、大人びていて、カップルが多め。パッラス=ユッラストゥントゥリ国立公園はフィンランド第3の広さです。',
    venues: {
